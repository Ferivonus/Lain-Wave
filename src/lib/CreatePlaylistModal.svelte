...
{#if playerState.isCreatePlaylistModalOpen}
    ...
    >
        <div 
            ...
            tabindex="-1"
        >
            <div class="flex border-b border-[var(--border)] bg-[var(--bg-card)] shrink-0">
                <button type="button" class="flex-1 py-4 text-[10px] font-black uppercase tracking-[0.2em] transition-all {mod === 'yeni' ? 'text-[var(--accent)] border-b-2 border-[var(--accent)] bg-[var(--bg-surface)]' : 'text-[var(--text-dim)] hover:text-white hover:bg-[var(--bg-card-hover)]'}" onclick={() => mod = 'yeni'}>
                    Sıfırdan Oluştur
                </button>
                <button type="button" class="flex-1 py-4 text-[10px] font-black uppercase tracking-[0.2em] transition-all {mod === 'import' ? 'text-[var(--accent)] border-b-2 border-[var(--accent)] bg-[var(--bg-surface)]' : 'text-[var(--text-dim)] hover:text-white hover:bg-[var(--bg-card-hover)]'}" onclick={() => mod = 'import'}>
                    JSON İçe Aktar
                </button>
            </div>
            ...
            <div class="overflow-y-auto custom-scrollbar flex-1 min-h-0">
                {#if mod === 'yeni'}
                    <div class="p-8 space-y-6" in:fade>
                        <div class="space-y-2">
                            <label for="pl-isim" class="text-[10px] font-black text-[var(--text-dim)] uppercase tracking-widest ml-1">Liste Adı</label>
                            <input id="pl-isim" type="text" bind:value={isim} use:focusAcilista onkeydown={(e) => e.key === 'Enter' && !yukleniyor && isim.trim() && kaydet()} placeholder="Örn: Gece Sürüşü, Retrowave..." class="w-full bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-4 text-sm outline-none focus:border-[var(--accent)]/50 transition-all font-bold placeholder:text-[var(--text-dim)]/30" />
                        </div>
                        <div class="flex gap-3 pt-4 border-t border-[var(--border)]">
                            <button type="button" onclick={kapat} class="flex-1 py-3.5 rounded-xl font-bold text-[10px] uppercase tracking-[0.2em] text-[var(--text-dim)] bg-[var(--bg-card)] hover:bg-[var(--bg-card-hover)] hover:text-white transition-all active:scale-95">İptal</button>
                            <button type="button" onclick={kaydet} disabled={yukleniyor || !isim.trim()} class="flex-1 py-3.5 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] text-white bg-[var(--accent)] hover:shadow-[0_0_20px_var(--accent-glow)] transition-all active:scale-95 disabled:opacity-30">
                ...
                {#if mod === 'import'}
                    <div class="p-6 md:p-8 flex flex-col h-full" in:fade>
                        {#if aramaModu}
                            <div class="space-y-5" in:fade>
                                <div class="flex items-center justify-between border-b border-[var(--border)] pb-4">
                                    <div>
                                        <h3 class="text-sm font-black text-[var(--accent)] uppercase tracking-widest">Manuel Frekans Arama</h3>
                                        <p class="text-[10px] text-[var(--text-dim)] font-bold mt-1.5 uppercase">Hedef: <span class="text-white">{aktifAramaSarki?.isim} - {aktifAramaSarki?.sarkici}</span></p>
                                    </div>
                                    <button type="button" onclick={() => aramaModu = false} class="px-4 py-2 border border-[var(--border)] bg-[var(--bg-card)] rounded-lg text-[10px] font-black uppercase tracking-widest text-[var(--text-dim)] hover:text-white hover:bg-[var(--bg-card-hover)] transition-all active:scale-95">İptal</button>
                                </div>

                                <div class="flex gap-3">
                                    <input type="text" bind:value={aramaSorgusu} onkeydown={(e) => e.key === 'Enter' && !aramaYapiliyor && baslatManuelArama()} class="flex-1 bg-[var(--bg-surface)] border border-[var(--border)] rounded-xl px-4 py-3 text-xs outline-none focus:border-[var(--accent)]/50 transition-all font-mono placeholder:text-[var(--text-dim)]/50" placeholder="Şarkı veya sanatçı adı..." />
                                    <button type="button" onclick={baslatManuelArama} disabled={aramaYapiliyor} class="bg-[var(--accent)] text-white px-6 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] hover:shadow-[0_0_15px_var(--accent-glow)] transition-all active:scale-95 disabled:opacity-50 min-w-[100px] flex items-center justify-center">
                                ...
                                {#if secilenSonuc}
                                    <div class="flex flex-col items-center justify-center p-8 border-2 border-[var(--accent)] rounded-2xl bg-[var(--accent)]/10 shadow-[0_0_30px_var(--accent-glow)]/30 transition-all" in:scale>
                                        ...
                                        <div class="flex gap-4 w-full max-w-sm">
                                            ...
                                            <button type="button" onclick={eslestirmeyiOnayla} class="flex-1 py-3.5 rounded-xl bg-[var(--accent)] text-white text-[10px] font-black uppercase tracking-widest shadow-lg hover:shadow-[0_0_20px_var(--accent-glow)] hover:scale-105 transition-all">Eşleştir</button>
                                        </div>
                                    </div>
                                {:else if aramaSonuclari.length > 0}
                                    <div class="max-h-[40vh] overflow-y-auto custom-scrollbar space-y-2 pr-2" in:fade>
                                        ...
                                        {#if gosterilenAramaSayisi < aramaSonuclari.length}
                                            <button type="button" onclick={() => gosterilenAramaSayisi += 5} class="w-full mt-2 py-4 rounded-xl border-2 border-dashed border-[var(--border)] text-[10px] font-black uppercase tracking-widest text-[var(--text-dim)] hover:text-[var(--accent)] hover:border-[var(--accent)]/50 hover:bg-[var(--accent)]/5 transition-all active:scale-95">
                        ...
                        {:else}
                            ...
                            <div class="flex-1 overflow-y-auto custom-scrollbar rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-2 space-y-1.5 my-4">
                                {#each importSarkilar as sarki, i}
                                    <div class="flex items-center p-3 rounded-lg bg-[var(--bg-surface)] border border-[var(--border)] text-xs transition-colors {sarki.durum === 'indiriliyor' ? 'border-[var(--accent)]/50 shadow-[0_0_10px_var(--accent-glow)]/10' : ''}">
                                        <div class="w-6 text-[10px] font-mono text-[var(--text-dim)]/50 font-bold shrink-0">{i + 1}</div>
                                        <div class="flex-1 min-w-0 pr-4">
                                            <p class="font-black truncate text-[var(--text-main)] {sarki.durum === 'indiriliyor' ? 'text-[var(--accent)]' : ''}">{sarki.isim}</p>
                                            <p class="text-[9px] uppercase tracking-[0.2em] text-[var(--text-dim)] font-bold truncate mt-0.5">{sarki.sarkici}</p>
                                        </div>
                                        <div class="shrink-0 flex items-center justify-end w-28">
                                            {#if sarki.durum === 'bekliyor'}
                                                <span class="text-[10px] font-black uppercase tracking-widest text-[var(--text-dim)]">Sırada</span>
                                            {:else if sarki.durum === 'indiriliyor'}
                                                <span class="text-[10px] font-black uppercase tracking-widest text-[var(--accent)] animate-pulse flex items-center gap-1.5"><svg class="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> İniyor</span>
                                            ...
                                            {:else if sarki.durum === 'hata' || sarki.durum === 'link_yok'}
                                                <button type="button" onclick={() => manuelAra(sarki)} class="bg-[var(--bg-card)] hover:bg-[var(--accent)] text-[var(--accent)] hover:text-white border border-[var(--accent)] px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-colors flex items-center gap-1.5 active:scale-95 shadow-sm hover:shadow-[0_0_10px_var(--accent-glow)]/50">
                            ...
                            <div class="flex gap-3 pt-2 shrink-0">
                                ...
                                {:else}
                                    <button type="button" onclick={kapat} disabled={isImporting} class="w-1/3 py-4 rounded-xl font-bold text-[10px] uppercase tracking-[0.2em] text-[var(--text-dim)] bg-[var(--bg-card)] hover:bg-[var(--bg-card-hover)] hover:text-white transition-all active:scale-95 disabled:opacity-30">
                                        Gizle
                                    </button>
                                    <button type="button" onclick={importBaslat} disabled={isImporting || bekleyenSayi === 0} class="flex-1 py-4 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] text-white bg-[var(--accent)] hover:shadow-[0_0_20px_var(--accent-glow)] transition-all active:scale-95 disabled:opacity-30 flex items-center justify-center gap-2">
                                        {#if isImporting}
