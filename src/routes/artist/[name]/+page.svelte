...
<div class="p-6 lg:p-10 w-full max-w-400 mx-auto min-h-full pb-32 flex flex-col min-w-0 bg-transparent text-(--text-main) transition-colors duration-500 overflow-y-auto custom-scrollbar">
  ...
  <section class="relative w-full min-h-87.5 lg:h-100 rounded-(--radius) overflow-hidden mb-12 shadow-2xl flex items-end p-8 lg:p-12 border border-(--border)" in:fade>
    <div class="absolute inset-0 z-0">
      ...
      {/if}
      <div class="absolute inset-0 bg-linear-to-t from-(--bg-main) via-(--bg-main)/40 to-transparent"></div>
    </div>

    <div class="relative z-10 flex flex-col md:flex-row items-center md:items-end gap-8 w-full">
      <div class="w-40 h-40 lg:w-56 lg:h-56 rounded-full overflow-hidden shadow-2xl border-4 border-(--border) shrink-0 bg-(--bg-surface) group">
        {#if kapakGorseli}
        ...
        {:else}
          <div class="w-full h-full bg-(--accent) flex items-center justify-center text-5xl font-black text-white">
            {sanatciAdi[0]}
          </div>
        {/if}
      </div>
      ...
      <div class="flex flex-col flex-1 text-center md:text-left min-w-0">
        <div class="flex items-center justify-center md:justify-start gap-2 text-(--accent) mb-3">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-6">
          ...
          {#if sanatciSarkilari.length > 0}
            <button 
              ...
              class="bg-(--text-main) text-(--bg-main) hover:bg-(--accent) hover:text-white px-10 py-3.5 rounded-full font-black text-[10px] lg:text-xs uppercase tracking-widest transition-all active:scale-95 shadow-xl"
            >
              Hepsini Oynat
            </button>
          {/if}
      ...
      <div class="flex flex-col gap-1">
        {#each sanatciSarkilari as sarki, i}
          <div 
            ...
            class="flex items-center p-3 rounded-2xl hover:bg-(--bg-card-hover) transition-all group cursor-pointer border border-transparent min-w-0 {playerState.aktifSarki?.id === sarki.id ? 'bg-(--accent)/5 border-(--accent)/20' : ''}"
          >
            <span class="w-8 text-center text-(--text-dim)/40 font-mono text-xs shrink-0 group-hover:hidden">{i + 1}</span>
            <div class="w-8 text-center hidden group-hover:block shrink-0">
               <svg class="w-4 h-4 mx-auto text-(--accent)" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5v14l11-7z"/></svg>
            </div>
            
            <div class="w-12 h-12 rounded-lg bg-(--bg-card) overflow-hidden shrink-0 mx-4 border border-(--border)">
              {#if sarki.kapak_yolu}
            ...
            <div class="flex-1 min-w-0 mr-4">
              <span class="block font-bold text-sm truncate {playerState.aktifSarki?.id === sarki.id ? 'text-(--accent)' : 'text-(--text-main)'}">
                {sarki.isim}
              </span>
              <span class="block text-[10px] text-(--text-dim) uppercase tracking-tight truncate">{sarki.album || 'Tekli'}</span>
            </div>
            ...
            <div class="flex items-center justify-end gap-2 px-2 shrink-0 w-48 sm:w-64" 
              ...
              <select aria-label="Listeye Ekle" onchange={(e) => handlePlaylistEkle(sarki.id, e)} class="bg-(--bg-surface) text-[9px] text-(--text-dim) rounded-lg px-1 py-1 outline-none border border-(--border) w-16 focus:border-(--accent) opacity-0 group-hover:opacity-100 hidden sm:block transition-all cursor-pointer font-bold">
                <option value="">➕</option>
              ...
              <button 
                  ...
                  class="p-2 text-(--text-dim)/60 hover:text-(--accent) hover:bg-(--accent)/10 rounded-lg transition-all opacity-0 group-hover:opacity-100 hidden sm:block"
              >
              ...
              <button 
                  ...
                  class="text-(--text-dim)/30 hover:text-red-500 transition-all p-2 opacity-0 group-hover:opacity-100"
              >
      ...
      <div class="grid grid-cols-2 lg:grid-cols-1 gap-6">
        {#each albumler as album}
          ...
          <div class="flex flex-col lg:flex-row items-center lg:items-start gap-4 group text-left w-full cursor-default">
            <div class="w-full lg:w-20 aspect-square bg-(--bg-card) rounded-2xl overflow-hidden relative border border-(--border) transition-all shadow-lg shrink-0">
              {#if albumKapak}
            ...
            <div class="flex flex-col min-w-0 py-1 items-center lg:items-start">
                <span class="text-[11px] font-black text-(--text-main) truncate w-full uppercase tracking-tighter mb-1 text-center lg:text-left">{album}</span>
                <span class="text-[9px] text-(--text-dim) font-bold uppercase tracking-widest">
