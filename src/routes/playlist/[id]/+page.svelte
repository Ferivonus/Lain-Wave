...
<div class="p-8 lg:p-12 w-full min-h-full pb-32 flex flex-col relative min-w-0 bg-transparent text-(--text-main) transition-colors duration-500 overflow-y-auto custom-scrollbar">
  ...
  <header class="flex flex-col md:flex-row gap-8 items-center md:items-end mb-12 mt-4" in:fade>
    <div class="w-52 h-52 lg:w-64 lg:h-64 bg-(--bg-card) rounded-(--radius) shadow-2xl shrink-0 border border-white/10 overflow-hidden relative group flex items-center justify-center">
        ...
        {:else}
          <div class="absolute inset-0 bg-linear-to-br from-(--accent)/20 to-(--bg-surface)"></div>
        {/if}
    ...
    <div class="flex flex-col text-center md:text-left pb-2 min-w-0 flex-1">
      <span class="text-[10px] font-black mb-3 text-(--accent) tracking-[0.4em] uppercase italic truncate">Kişisel Frekans Ağı</span>
      ...
      <div class="flex flex-col sm:flex-row items-center gap-6 mt-2">
        <p class="text-(--text-dim) text-xs lg:text-sm font-bold uppercase tracking-widest whitespace-nowrap truncate">
        ...
        <div class="flex items-center gap-3">
            ...
            {#if gosterilenSarkilar.length > 0}
              <button 
                type="button"
                onclick={listeyiCal} 
                class="flex items-center gap-3 bg-(--text-main) text-(--bg-main) hover:bg-(--accent) hover:text-white px-10 py-2.5 rounded-lg font-black shadow-[0_5px_15px_rgba(0,0,0,0.3)] hover:shadow-[0_0_20px_var(--accent-glow)] transition-all active:scale-95 uppercase text-[10px] lg:text-xs tracking-widest shrink-0"
  ...
  {:else}
    <div class="sticky top-0 z-20 flex items-center text-[10px] font-black text-(--text-dim) border-b border-(--border) pb-3 mb-4 px-6 pt-4 bg-(--bg-main)/95 backdrop-blur-xl tracking-[0.2em] uppercase shrink-0 rounded-t-xl shadow-sm">
      ...
      {#each gosterilenSarkilar as sarki, index}
        <div 
            ...
            class="flex items-center p-2.5 px-6 rounded-2xl transition-all duration-300 group cursor-pointer border-t-2
                   ...
                   {uzerindeGezinilenIndex === index ? 'border-(--accent)! bg-(--accent)/10 shadow-lg scale-[1.01]' : ''}"
        >
          <div class="w-12 shrink-0 flex items-center justify-start relative">
             <div class="mr-2 text-(--text-dim)/20 hover:text-(--accent) cursor-grab active:cursor-grabbing opacity-0 group-hover:opacity-100 transition-all shrink-0" title="Sıralamayı Değiştir">
             ...
             <div class="font-mono text-xs text-(--text-dim)/40 flex items-center justify-center w-4">
                 ...
                 {:else}
                    ...
                    <svg class="w-4 h-4 hidden group-hover:block text-(--accent) absolute left-6" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                 {/if}
             </div>
          </div>
          
          <div class="flex-1 min-w-0 flex items-center gap-4 ml-4">
            <div class="w-11 h-11 bg-(--bg-card) rounded-lg overflow-hidden shrink-0 shadow-md border border-(--border)">
              {#if sarki.kapak_yolu}
            ...
            <div class="flex flex-col min-w-0 flex-1 pr-2">
              <span class="font-black text-sm lg:text-base truncate tracking-tight {playerState.aktifSarki?.id === sarki.id ? 'text-(--accent)' : 'text-(--text-main)'}">
                {sarki.isim}
              </span>
              <div class="flex items-center gap-2 mt-0.5 overflow-hidden">
                <a href="/artist/{encodeURIComponent(sarki.sarkici)}" onclick={(e) => e.stopPropagation()} class="text-[10px] text-(--text-dim) truncate font-bold uppercase tracking-widest hover:text-(--accent) transition-colors inline-block max-w-max opacity-80 text-left">
                  {sarki.sarkici}
                </a>
                {#if sarki.album}
          ...
          <div class="w-52 sm:w-64 flex items-center justify-end gap-2 shrink-0" role="presentation" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()}>
            ...
            <button 
                ...
                class="p-2 text-(--text-dim)/60 hover:text-orange-400 hover:bg-orange-400/10 rounded-lg transition-all"
            >
