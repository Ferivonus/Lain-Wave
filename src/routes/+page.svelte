...
<div class="p-8 lg:p-10 w-full min-h-full pb-32 flex flex-col relative min-w-0 overflow-y-auto custom-scrollbar bg-transparent text-[var(--text-main)] transition-colors duration-500">
    ...
    {#if playerState.sarkiListesi.length === 0}
        <div class="flex flex-col items-center justify-center flex-1 mt-10 p-10 bg-[var(--bg-card)] border border-[var(--border)] rounded-[var(--radius)] border-dashed" in:fade>
            ...
            <p class="text-[var(--text-dim)] mb-8 max-w-md text-center font-medium">Arşivin henüz boş. Bilgisayarından veya dış kaynaklardan yeni parçalar aktararak kütüphaneni oluşturmaya başla.</p>
            <button type="button" onclick={() => playerState.isAddMusicModalOpen = true} class="bg-[var(--accent)] hover:opacity-90 text-white px-8 py-3 rounded-full font-black shadow-[0_10px_30px_var(--accent-glow)] transition-all hover:scale-105 active:scale-95 flex items-center gap-2 tracking-widest uppercase text-xs">
    ...
    {:else}
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8" in:fly={{ y: -20, duration: 600 }}>
            ...
            <div class="flex gap-3">
                ...
                <div class="bg-[var(--bg-surface)] border border-[var(--border)] px-4 py-2 rounded-xl flex items-center gap-2 text-[10px] font-black text-[var(--text-dim)] tracking-[0.2em] uppercase shadow-sm">
                    <svg class="w-4 h-4 text-[var(--accent-sec)]" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>
        ...
        {#if gununSarkisi}
            <div class="w-full relative rounded-[var(--radius)] overflow-hidden mb-12 group shadow-2xl border border-[var(--border)]" in:fade>
                <div class="absolute inset-0 bg-gradient-to-r from-[var(--bg-main)] via-[var(--bg-main)]/80 to-transparent z-10"></div>
                ...
                {:else}
                    <div class="absolute inset-0 bg-gradient-to-br from-[var(--accent)]/30 to-[var(--accent-sec)]/30"></div>
                {/if}
                ...
                <div class="relative z-20 p-8 lg:p-10 flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-10">
                    <div class="w-32 h-32 md:w-48 md:h-48 shrink-0 rounded-2xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 relative group-hover:shadow-[0_20px_50px_var(--accent-glow)] transition-all duration-500">
                        ...
                        {:else}
                            <div class="w-full h-full bg-[var(--bg-surface)] flex items-center justify-center text-[var(--text-dim)]">
                    ...
                    <div class="flex flex-col flex-1 text-center md:text-left">
                        <span class="text-[10px] font-black tracking-[0.4em] text-[var(--accent)] uppercase mb-3 drop-shadow-md">Günün Öne Çıkanı</span>
        ...
        {#if hizliErisim.length > 0}
            <div class="mb-12">
                <h2 class="text-[11px] font-black text-[var(--text-dim)] uppercase tracking-[0.3em] mb-5">Hızlı Erişim</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                    {#each hizliErisim as sarki}
                        <div 
                            ...
                            class="flex items-center bg-[var(--bg-card)] hover:bg-[var(--bg-card-hover)] border border-[var(--border)] hover:border-[var(--accent)]/40 rounded-xl overflow-hidden cursor-pointer group transition-all duration-300 shadow-md active:scale-[0.98]"
                        >
                            <div class="w-16 h-16 bg-[var(--bg-surface)] shrink-0 relative border-r border-[var(--border)]">
                                {#if sarki.kapak_yolu}
                                ...
                                <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                    <div class="w-8 h-8 bg-[var(--accent)] rounded-full flex items-center justify-center shadow-lg transform scale-75 group-hover:scale-100 transition-all text-white">
        ...
        {#if favoriSanatcilar.length > 0}
            <div class="mb-14">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-[11px] font-black text-[var(--text-dim)] uppercase tracking-[0.3em]">Sık Dinlediğin Sanatçılar</h2>
                    <a href="/artists" class="text-[10px] font-black text-[var(--accent)] hover:text-[var(--text-main)] transition-colors uppercase tracking-widest bg-[var(--accent)]/10 px-3 py-1.5 rounded-lg border border-[var(--accent)]/20">Tümünü Gör</a>
                </div>
        ...
        {#if enCokDinlenenler.slice(0,5).length > 0}
            <div class="mb-14">
                <h2 class="text-[11px] font-black text-[var(--text-dim)] uppercase tracking-[0.3em] mb-6">Senin İçin Zirvedekiler</h2>
                    ...
                    {#each enCokDinlenenler.slice(0,5) as sarki}
                        ...
                        >
                            <div class="w-full aspect-square bg-[var(--bg-surface)] rounded-xl mb-4 relative overflow-hidden shadow-inner border border-[var(--border)] group-hover:border-[var(--accent)]/30 transition-colors">
                                ...
                                <div class="absolute bottom-3 right-3 w-12 h-12 bg-[var(--accent)] text-white rounded-full flex items-center justify-center opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 shadow-[0_10px_20px_rgba(0,0,0,0.5)]">
                                    <svg class="w-5 h-5 ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                                </div>
                            </div>
                            <div class="flex flex-col min-w-0 flex-1">
                                <span class="font-bold text-sm lg:text-base truncate mb-1 group-hover:text-[var(--accent)] transition-colors">{sarki.isim}</span>
                                <span class="text-[var(--text-dim)] font-bold text-[9px] uppercase tracking-widest truncate opacity-70 mb-3">{sarki.sarkici}</span>
                                <span class="text-[9px] text-[var(--accent)] bg-[var(--accent)]/10 border border-[var(--accent)]/20 px-2 py-1 rounded-md font-black tracking-widest mt-auto w-fit uppercase">{sarki.dinlenme_sayisi} Dinlenme</span>
                            </div>
                        </div>
                    {/each}
                </div>
        ...
        {#if yeniEklenenler.length > 0}
            <div class="mb-4">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-[11px] font-black text-[var(--text-dim)] uppercase tracking-[0.3em]">Yeni Eklenenler</h2>
                    <a href="/library" class="text-[10px] font-black text-[var(--accent-sec)] hover:text-[var(--text-main)] transition-colors uppercase tracking-widest bg-[var(--accent-sec)]/10 px-3 py-1.5 rounded-lg border border-[var(--accent-sec)]/20">Tümünü Gör</a>
                </div>
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                    {#each yeniEklenenler as sarki}
                        ...
                        >
                            <div class="w-full aspect-square bg-[var(--bg-surface)] rounded-xl mb-4 relative overflow-hidden shadow-inner border border-[var(--border)] group-hover:border-[var(--accent-sec)]/30 transition-colors">
                                ...
                                {/if}
                                <div class="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors duration-500"></div>
                                <div class="absolute bottom-3 right-3 w-12 h-12 bg-[var(--accent-sec)] text-white rounded-full flex items-center justify-center opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 shadow-[0_10px_20px_rgba(0,0,0,0.5)]">
                                    <svg class="w-5 h-5 ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                                </div>
                            </div>
                            
                            <div class="flex flex-col min-w-0 flex-1">
                                <span class="font-bold text-sm lg:text-base truncate mb-1 group-hover:text-[var(--accent-sec)] transition-colors">{sarki.isim}</span>
                                <span class="text-[var(--text-dim)] font-bold text-[9px] uppercase tracking-widest truncate opacity-70 mb-3">{sarki.sarkici}</span>
                                
                                <div class="mt-auto flex gap-2 overflow-hidden">
                                    {#if sarki.kalite && sarki.kalite.trim() !== ""}
                                        <span class="text-[8px] px-1.5 py-0.5 rounded bg-[var(--bg-surface)] text-[var(--text-dim)] border border-[var(--border)] font-black uppercase tracking-widest shrink-0">{sarki.kalite}</span>
                                    {/if}
                                    {#if sarki.tarz}
                                        <span class="text-[8px] px-1.5 py-0.5 rounded bg-[var(--accent-sec)]/10 text-[var(--accent-sec)] border border-[var(--accent-sec)]/20 font-black uppercase tracking-widest truncate">{sarki.tarz}</span>
                                    {/if}
                                </div>
                            </div>
                        </div>
